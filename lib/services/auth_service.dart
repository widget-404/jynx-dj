...
class AuthServices {
  Future<String> registerUser(String firstName, lastName, phoneNumber, email,
      password, confirmPassword, musicGenre, gender) async {
    ...
    print(jsonMap);
    final response = await http.post(Uri.parse("${ip.testIP}/register_djuser"),
        body: jsonEncode(jsonMap));
    print(
        "I am receiving Response of URL http://kaspar.eastus.cloudapp.azure.com/jynx_club/api/register_djuser ${response.body} ");
  ...
  Future<String> loginUser(String email, password) async {
    ...
    print(jsonMap);
    final response = await http.post(Uri.parse("${ip.testIP}/login_djuser"),
        body: jsonEncode(jsonMap));
    ...
    if (response.statusCode == 201) {
      ...
      if (responseMusicGenre['status'] == '0') {
        ...
        await prefs.setString('user_name', responseMusicGenre['name']);
        await prefs.setString('user_id', responseMusicGenre['id'].toString());
        return responseMusicGenre['id'].toString();
      } else {
        return '0';
      }
  ...
  Future<String> updatePassword(String email, password) async {
    ...
    print(jsonMap);
    final response = await http.post(Uri.parse("${ip.testIP}/update_password"),
        body: jsonEncode(jsonMap));
    ...
    if (response.statusCode == 200) {
      ...
      if (responseupdatePassword['message'] ==
